...
class AboutGenerators(Koan):
    ...
    def test_generators_can_act_as_coroutines(self):
        ...
        """ When a generator is first created, there is no yield expression to
            recieve a value. So, if you call next(), a TypeError will be raised.

                TypeError: can't send non-None value to a just-started generator

            So, in order to use a coroutine, you must iterate once to advance its
            execute to the first yield expression. """
        next(generator)
